...
app.get("/log", function(req, res){
    streamLogs(self.logRing, res);
});
app.get("/log/:id", function(req, res){
    if (self.workers[req.params.id]) {
        streamLogs(self.workers[req.params.id].logRing, res);
    } else {
        res.type("text");
        res.send(404, "no worker found with that id");
    }
});
...
function streamLogs(logRing, res) {
    //TODO: eventstream
    res.type("text");
    var handler = function(message) {
        res.write(message);
    };
    logRing.attach(handler);

    res.on("finish", function(){
        logRing.detach(handler);
    });
...
function sse(data) {
    return data.toString().replace(/^/m, "data: ") + "\n";
}
